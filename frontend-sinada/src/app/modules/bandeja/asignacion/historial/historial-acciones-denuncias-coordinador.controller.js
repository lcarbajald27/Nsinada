(function() {
	...
	.controller('HistorialAccionesDenunciasCoordinadorController',HistorialAccionesDenunciasCoordinadorController);


	/** @ngInject */
	function HistorialAccionesDenunciasCoordinadorController($window, $scope,$state, toastr, ngDialog,MaestroFactory,InformeAccionFactory,$filter,BandejaFactory,$rootScope,AtencionDenuncia, EvaluacionRechazoFactory,EvaluacionInformeFactory) {
		var vm = this;
		...
		vm.levantarObservacionAtencion=levantarObservacionAtencion;
				...
				for(var i = 0 ; i<prmFileList.length;i++){
					...
					//	console.log(archivo);
						...
						if (archivo.name.length>50) {
							...
							toastr.warning('El nombre del archivo no debe ser mayor a 50 caracteres');
							 document.getElementById("fileArchivoMedio").value = "";
							return;
       ...
       {		
		...
		/*	function listarAccionesRegistradas(){
			...
			AtencionDenuncia
						.listarAccionPorIdDenuncia(data)
						.then(function (response) {
							if (response.valido) {
								vm.listaAccionesRegistradas= angular.copy(response.data);
								//console.log(" vm.listaAccionesRegistradas" + vm.listaAccionesRegistradas);
	 ...
	 function registrarAtencion()
       {		
		...
		function init() {
			...
			if($rootScope.dataHistorialDenuncia!= null){

					vm.dataDenunciaBandeja = $rootScope.dataHistorialDenuncia;
