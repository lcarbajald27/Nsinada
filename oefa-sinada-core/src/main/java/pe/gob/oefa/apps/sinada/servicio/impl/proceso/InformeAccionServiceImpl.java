...
@Service("informeAccionService")
@Transactional(readOnly = true)
public class InformeAccionServiceImpl implements InformeAccionService {
	
	@Autowired
	private InformeAccionDAO informeAccionDAO;
	...
	private static final long ID_REGISTRO = 26;
	
	
	@Override
	public Long insertar(InformeAccion prmBean) throws ServicioException {
		try{
			Long idBean =informeAccionDAO.insertar(prmBean);
	...
	@Override
	public int actualizar(InformeAccion prmBean) throws ServicioException {
		try{
	/********************* Auditoria ***************************/
			
			long idAuditoria = 0;
			InformeAccion prmDataOld = obtenerDatosAntiguos(prmBean);
				...
				auditoria.setIdUsuario(Long.valueOf(PK_eIdUsuarioDecrypt));
				auditoria.setHostIp(prmBean.getHostIp());
				auditoria.setIdRegistro(prmBean.getIdInformeAccion());
				
				idAuditoria = auditoriaService.insertar(auditoria);
			
				if(idAuditoria!=0){
					
					compararAntiguoNuevo(String.valueOf(prmBean.getEstado()),String.valueOf(prmDataOld.getEstado()),idAuditoria,236);
		
				}
			
			/*********************************************/
			
			return informeAccionDAO.actualizar(prmBean);
		}catch (Exception e){
			throw new ServicioException(e);
		}
	}
	...
	@Override
	public int eliminar(InformeAccion prmBean) throws ServicioException {
		try{
			
			/******************* Auditoria ****************************/
			if(prmBean.getIdInformeAccion()!=0){
				String PK_eIdUsuarioDecrypt = "0";
				if(prmBean.getPrm1()!=null){
					UtilEncrypt.init(UtilProperties.getKey("encrypt.key"));
			...
			return informeAccionDAO.eliminar(prmBean);
		}catch (Exception e){
			throw new ServicioException(e);
		}
	}
	...
	@Override
	public InformeAccion buscarPorId(Long prmIdBean) throws ServicioException {
		// TODO Auto-generated method stub
		try {
			return informeAccionDAO.buscarPorId(prmIdBean);
		} catch (Exception e) {
			throw new ServicioException(e);
		}
	}
	...
	@Override
	public List<?> listar(InformeAccion prmBean) throws ServicioException {
		// TODO Auto-generated method stub
		try {
			return informeAccionDAO.listar(prmBean);
		} catch (Exception e) {
			throw new ServicioException(e);
		}
	}
	
	public InformeAccion obtenerDatosAntiguos(InformeAccion prmData){
//		int id=Integer.valueOf(String.valueOf(prmData.getIdEfa()));
		InformeAccion oBean = null;
		
				try {
					oBean = informeAccionDAO.buscarPorId(prmData.getIdInformeAccion());
